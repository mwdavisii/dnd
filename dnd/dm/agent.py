...
from dnd.ui import style, wrap_text
...
class DungeonMaster:
    ...
    def generate_response(self, prompt: str, player_sheet: CharacterSheet, npcs: dict) -> str:
        ...
        try:
            ...
            self.add_history("assistant", final_response)
            print(style(self._format_narration(final_response), "parchment"))
    ...
    def _format_narration(self, text: str) -> str:
        ...
        if len(paragraphs) <= 1:
            return self._style_quotes(self._bold_names(wrap_text(text), names_to_bold))

        labels = ["[Scene]", "[Problem]", "[What Stands Out]"]
        formatted = []
        for index, paragraph in enumerate(paragraphs):
            label = labels[index] if index < len(labels) else None
            wrapped = self._style_quotes(self._bold_names(wrap_text(paragraph), names_to_bold))
            if label:
    ...
    def _style_quotes(self, text: str) -> str:
        return re.sub(
            r'"([^"\n]+)"',
            lambda match: style(f'"{match.group(1)}"', "quote", italic=True),
            text,
        )
