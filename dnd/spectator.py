...
def validate_turn_output(
    ...
    if not cleaned:
        return fallback or default_fallback_action(actor_name, actor_type)
...
def default_fallback_action(actor_name: str, actor_type: str) -> str:
    if actor_type == "companion":
        return "I keep watch, cover the group, and point out the next safe opening."
    return f"{actor_name} studies the scene, moves toward the clearest lead, and stays ready to react."


def suggest_objective_action(actor_name: str, actor_type: str, turn_context: dict | None = None) -> str:
    if not turn_context:
        return default_fallback_action(actor_name, actor_type)
    ...
    if actor_type == "companion":
        if notable_npcs:
            return f"I focus on {notable_npcs[0]}, stay on the main lead, and point out the safest next move."
        return "I stay on the main lead, cover the group, and point out the next opening."
    if nearby_locations:
        return f"{actor_name} stays on the main lead, moves toward {nearby_locations[0]}, and looks for the fastest way to advance."
    if objective:
        return f"{actor_name} commits to the active objective, presses for a key answer, and keeps the scene moving."
    return default_fallback_action(actor_name, actor_type)
